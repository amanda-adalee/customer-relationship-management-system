...
/**In this class the user can update/modify customer information. Previous customer details are displayed in text fields
 and combo boxes, which the user can modify to reflect the new customer details, then save.*/
public class CustomerModifyController implements Initializable {
    ...
    @FXML
    void backUpdateCustomerButton(ActionEvent event) throws IOException {
    ...
    /**This button saves the updated customer to the database. Before saving, an input validating function is called.
     If the database is updated with the updated customer then the customer menu scene will load. If the update is not
    ...
    void updateCustomerButton(ActionEvent event) throws IOException {
        ...
        if (isInputValid()){
            ...
            boolean result = CustomerDB.updateCustomer(newCustomer);
            if (result)
            {
                stage = (Stage)((Button)event.getSource()).getScene().getWindow();
                scene = FXMLLoader.load(getClass().getResource("/view/CustomerMenu.fxml"));
                ...
                errorAlert.setContentText("Error updating the database. Please try again.");
    ...
    /**This function sets the text for the text fields and for the combo boxes.
     @param customer this customer object was selected by the user in the customer menu. It has been passed
     in order to make changes to the customer.*/
    public void sentCustomer(Customer customer, int index) {
    ...
    @FXML
    public void onActionCountryComboBox(ActionEvent event) {
